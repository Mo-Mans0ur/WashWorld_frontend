...
export default function HandleSubscriptionPage() {
  ...
  return (
    <>
      <AppHeader />
      <section
        className="min-h-[calc(100dvh-88px-112px)] px-6 pt-3.5 pb-4 text-white"
        style={{
          background: "linear-gradient(90deg, #75cfa0 0%, #8f9994 100%)",
        }}
      >
        <h1 className="text-center text-5.5 font-bold leading-tight">
          Opret
          <br />
          vaskeabonnement
        </h1>
        <p className="mx-auto mt-1 max-w-60 text-center text-3 font-semibold leading-tight">
          Hold din bil ren med et vaskeabonnement og spar penge
        </p>

        <div className="mx-auto mt-3.5 w-full max-w-74 bg-(--brand-green-01) px-4 py-2.5 text-center shadow-lg">
          <h2 className="text-9 font-extrabold leading-none">
            {activePlan.name}
          </h2>
          <p className="mt-1 text-4.75 font-extrabold leading-none">
            {activePlan.monthly}
          </p>
          <p className="mt-1 text-3.25 font-extrabold leading-none">
            {activePlan.firstMonth}
          </p>
        </div>

        <p className="mx-auto mt-2 max-w-72 text-center text-2.5 leading-tight">
          *Gælder kun nummerplader, der ikke før har haft et abonnement hos Wash
          world.
        </p>

        <p className="mx-auto mt-7 max-w-66 text-center text-3.5 font-bold leading-tight">
        ...
        <div className="mx-auto mt-3.5 flex w-full max-w-72 flex-col gap-1.5">
          <div className="relative flex h-9.5 items-center overflow border border-(--color-grey-02) bg-(--white-white) text-black">
            ...
            <span className="flex-1 truncate pr-20 text-3.5 font-semibold text-(--color-grey-01)">
              {vehicle
            ...
            <button
              ...
              className="absolute top-2.25 -right-px -bottom-px flex min-w-19 items-center justify-center bg-(--brand-green-01) px-3 text-center text-3.5 font-bold text-white [clip-path:polygon(16%_0,100%_0,100%_100%,0_100%)]"
            >
              Vælg
            </button>
          </div>
          ...
          <div className="relative flex h-9.5 items-center overflow border border-(--color-grey-02) bg-(--white-white) text-black">
            ...
            <span className="flex-1 pr-20 text-3.5 font-semibold text-(--color-grey-01)">
        ...
        <label className="mx-auto mt-2.5 flex w-full max-w-72 items-center justify-center gap-2 text-2.5 font-semibold text-(--white-white)">
          <input
            type="checkbox"
        ...
        <div className="mx-auto mt-5.5 flex w-full max-w-69 items-center gap-0">
          <button
            type="button"
            onClick={() => router.back()}
            className="h-8.5 flex-1 bg-(--color-grey-01) text-5.5 font-extrabold text-white [clip-path:polygon(0_0,100%_0,88%_100%,0_100%)]"
          >
            Tilbage
          </button>
          ...
          <button
            ...
            disabled={!canSubmit}
            className="-ml-3.5 h-8.5 flex-1 bg-(--brand-green-01) text-5.5 font-extrabold text-white [clip-path:polygon(12%_0,100%_0,100%_100%,0_100%)]"
          >
            Opret
          </button>
        </div>
