...
const DROPS = [
  ...
  { x: "64%", delay: 0.30, dur: 0.68, w: 3.0, h: 7  },
...
const RIVULETS = [
  { x: "19%", y: 18, len: 65, delay: 0.00, dur: 0.90 },
  { x: "33%", y: 12, len: 55, delay: 0.35, dur: 0.82 },
  { x: "50%", y: 10, len: 72, delay: 0.68, dur: 1.00 },
  { x: "67%", y: 14, len: 58, delay: 0.18, dur: 0.88 },
  { x: "81%", y: 20, len: 48, delay: 0.50, dur: 0.94 },
  { x: "26%", y: 70, len: 45, delay: 0.80, dur: 0.85 },
...
function Nozzle({ active }: { active: boolean }) {
  return (
    <svg viewBox="0 0 280 34" style={{ display: "block", width: "100%" }} aria-hidden="true">
      <rect x="112" y="4" width="56" height="20" rx="6"
        fill="white" opacity={active ? 0.88 : 0.20}
        style={{ transition: "opacity 0.5s" }} />
      {[123, 131, 140, 149, 157].map((x) => (
        <circle key={x} cx={x} cy="19" r="3.2"
          fill="rgba(0,26,8,0.62)"
          opacity={active ? 0.90 : 0.22}
          style={{ transition: "opacity 0.5s" }} />
      ))}
      <rect x="128" y="0" width="24" height="6" rx="3"
        fill="white" opacity={active ? 0.55 : 0.10}
        style={{ transition: "opacity 0.5s" }} />
    </svg>
  );
}
...
function CarIllustration({ stage, progress }: { stage: WashStage; progress: number }) {
      ...
      <div className="relative mx-auto w-full" style={{ maxWidth: 300 }}>
        ...
        <Image
          ...
          style={{
            ...
            mixBlendMode: "screen",
            filter: showClean
              ? `brightness(${1 + cleanOpacity * 0.20})`
              : dirtOpacity > 0
          ...
          {dirtOpacity > 0 && (
            <svg style={{ position: "absolute", inset: 0, width: "100%", height: "100%" }}
              ...
              <g opacity={dirtOpacity} style={{ transition: "opacity 0.7s" }}>
                ...
                <path d="M 120,90 Q 124,112 122,134"
                  stroke="rgba(0,0,0,0.22)" strokeWidth="3" fill="none" strokeLinecap="round" />
                <path d="M 178,88 Q 182,110 180,132"
                  stroke="rgba(0,0,0,0.20)" strokeWidth="2.5" fill="none" strokeLinecap="round" />
                <path d="M 112,72 Q 132,60 154,56"
